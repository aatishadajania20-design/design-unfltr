  ...
  return (
    <>
      <style>{`
        ...
        /* ── WORDMARK ── same visual size as 26px logo ── */
        .sh-wordmark {
          font-size: 1.1rem;
        ...
        @media(min-width: 768px) { .sh-wordmark { font-size: 1.2rem; } }

        /* Every letter shares one class — --i drives the wave stagger */
        .sh-letter {
          display: inline-block;
          color: #fff;
          transition: color 0.22s ease, transform 0.36s cubic-bezier(0.34,1.56,0.64,1);
          transition-delay: 0ms;
        }
        .sh-letter-studio { color: #f97316; }

        /* Wave bounce through all 12 letters on hover */
        ...
        .sh-nav-top, .sh-nav-bottom {
          display: block; font-size: 0.78rem; font-weight: 500;
          letter-spacing: 0.08em; color: #9ca3af; line-height: 1;
        ...
        /* 
          The blade sits at rotate(-14deg).
          Text split: top half slides up-right, bottom half slides down-left.
          skewX(14deg) on both halves — same sign as blade tilt.
          Translation: purely vertical (up/down) so the split gap 
          opens perpendicular to the text baseline, not sideways.
          This makes it look like the blade cut straight through the word.
        */
        .sh-nav-item:hover .sh-nav-top    { transform: translateY(-10px) skewX(14deg); color:#fff; }
        .sh-nav-item:hover .sh-nav-bottom { transform: translateY(10px)  skewX(14deg); color:#fff; }

        /* ── KATANA BLADE ── */
        .sh-blade {
          position: absolute;
          top: 50%; left: -18px; right: -18px; height: 2px;
          transform: translateY(-50%) scaleX(0) rotate(-14deg);
          transform-origin: left center;
          /* Fast aggressive entry, smooth settle */
          transition: transform 0.32s cubic-bezier(0.08, 0.9, 0.2, 1);
        ...
        /* Polished steel with orange glow, tapered tip */
        .sh-blade-body {
          flex: 1; height: 100%;
          background: linear-gradient(
            ...
            #ffb347 42%,
            #f97316 72%,
            rgba(255,200,100,0.45) 94%,
            transparent 100%
          );
          clip-path: polygon(0% 0%, 97% 22%, 100% 50%, 97% 78%, 0% 100%);
          box-shadow: 0 0 4px rgba(249,115,22,0.65), 0 0 10px rgba(249,115,22,0.22);
          border-radius: 0 1px 1px 0;
        }

        /* Flash races ahead of the blade */
        .sh-blade-flash {
        ...
        .sh-nav-item:hover .sh-blade-tip {
          animation: sh-tip 0.42s ease 0.24s forwards;
        }
        @keyframes sh-tip {
